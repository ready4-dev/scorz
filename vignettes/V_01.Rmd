...
## Ingest data
To begin, we ingest an unscored dataset as an instance of the `Ready4useRecord` class (from the [ready4use package](https://ready4-dev.github.io/ready4use/)). In this case we download our data from a remote repository.
...
We can inspect our ingested dataset using the `exhibit` method.
...
By default, instances of the `ScorzAqol6Adol` class are created with a slot specifying a default value for the prefix for AQoL-6D questionnaire item responses.
```{r}
z@itm_prefix_1L_chr
```
...
We extract our updated dataset, along with its data-dictionary using the `procureSlot` method.
...
We can summarise all individual items by timepoint.

```{r}
depict(z, type_1L_chr = "item_by_time")
```

We can generate plots of selected items by passing their index number to the `var_idcs_int` argument.
...
```
Total AQoL-6D scores can also be plotted.
```{r}
